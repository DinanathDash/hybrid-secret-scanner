...
def _apply_redaction(candidate: CandidateSecret, raw_context: str, context_lines: List[str]) -> CandidateSecret:
    """Applies the strict redaction string and attempts to extract variable name."""
    redaction_string = f"<REDACTED_SECRET_LENGTH_{len(candidate.raw_secret)}_ENTROPY_{candidate.entropy:.2f}>"
