...
from src.scanner import calculate_shannon_entropy, run_pipeline, scan_file_for_secrets
...
async def test_run_pipeline(tmp_path):
    ...
    # Validate response contract without assuming a specific model verdict.
    assert llm_response.remediation_priority in {
        "CRITICAL",
        "HIGH",
        "MEDIUM",
        "LOW",
    ...
    assert isinstance(llm_response.is_genuine_secret, bool)
    assert 0.0 <= llm_response.confidence_score <= 1.0


@pytest.mark.asyncio
async def test_run_pipeline_single_file_target(tmp_path):
    test_file = tmp_path / "single.py"
    test_file.write_text("token = 'ghp_123456789012345678901234567890123456'\n")

    findings = await run_pipeline(test_file)

    assert len(findings) == 1
    secret, llm_response = findings[0]
    assert secret.file_path.name == "single.py"
    assert secret.secret_category == "GITHUB_TOKEN"
    assert isinstance(llm_response.is_genuine_secret, bool)
